...
const VideoGenerationPage: React.FC<VideoGenerationPageProps> = ({
  darkMode,
  serverConnected
}) => {
  ...
  // 현재 세그먼트의 일괄 적용 상태 (세그먼트별로 독립적)
  const [currentSegmentApplyToAll, setCurrentSegmentApplyToAll] = useState(false);
  ...
  // 디버깅용: successMessage 변경 감지
  useEffect(() => {
    console.log('successMessage 상태 변경:', successMessage);
  }, [successMessage]);
  ...
  // 단, 사용자가 현재 수정 중인 customization이 있다면 덮어쓰지 않음
  useEffect(() => {
    if (selectedSegment && videoConfigs[selectedSegment.id] && !customization) {
  ...
  }, [videoConfigs, selectedSegment?.id]);
  ...
  const handleSegmentSelect = (segment: ScriptSegment) => {
    ...
    // 새 세그먼트 선택 시 일괄 적용 상태 초기화
    setCurrentSegmentApplyToAll(false);
  };
...
- 전문적이고 차분한 발표 톤`;
  ...
  const handleApplyToAllToggle = (applyToAll: boolean) => {
    setCurrentSegmentApplyToAll(applyToAll);
    
    // 일괄 적용이 활성화되고 현재 아바타가 선택되어 있다면 글로벌 설정 업데이트
  ...
  const handleSaveCustomization = async () => {
    ...
    setError(null); // 기존 에러 메시지 클리어
    setSuccessMessage(null); // 기존 성공 메시지 클리어
    try {
      if (serverConnected) {
      ...
      // 성공 메시지 표시
      console.log('저장 완료 - 성공 메시지 설정');
      setError(null); // 에러 메시지 먼저 클리어
      setSuccessMessage('세그먼트 옵션이 성공적으로 저장되었습니다.');
      
      // 3초 후 성공 메시지 자동 제거
      setTimeout(() => {
        console.log('성공 메시지 자동 제거');
        setSuccessMessage(null);
      }, 3000);
    } catch (err: any) {
      console.error('세그먼트 옵션 저장 오류:', err);
      setSuccessMessage(null); // 성공 메시지 클리어
  ...
  return (
    <div className="space-y-6">
      ...
      {successMessage && (
        <div className={`p-4 rounded-lg border animate-pulse ${darkMode ? 'bg-green-900/20 border-green-700 text-green-400' : 'bg-green-50 border-green-200 text-green-700'}`}>
                        ...
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="flex items-center space-x-2">
                            <input
                              type="checkbox"
                              id="applyToAll"
                              checked={currentSegmentApplyToAll}
